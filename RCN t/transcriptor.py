...
def creacionDeTranscripciones():
    ...
    blobs = clientStorage.list_blobs(BUCKET_NAME, prefix='RCN')
    for archivo in blobs:
        if "." in archivo.name:
            ...
            if (año == '2016') & (mes == 'Abril'):
                dia = parametros[3]
                audio = parametros[4]
                # crea los directorios para guardar las transcripciones
                os.makedirs(dir_transcripciones + '/{}/{}/{}'.format(año, mes, dia), exist_ok=True)
                # ruta dónde está el audio
                rutaAudio = dirAudios + '/{}/{}/{}/{}'.format(año, mes, dia, audio)
                #ruta dónde se guarda el archivo
                rutaDestino = dir_transcripciones + '/{}/{}/{}/{}.txt'.format(año, mes, dia, audio.split(".")[0])
